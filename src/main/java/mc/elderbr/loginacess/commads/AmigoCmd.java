...
public class AmigoCmd implements CommandExecutor, Comando, JogadorInterface {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (sender instanceof Player player) {
            ...
            switch (myCommand) {
                case "cadastro":
                    return cadastro();// Se for amigo e não for cadastrado tem que fazer
                case "login":
                    return login();// Se for amigo precisa fazer o login
                case "addamigo":
                    return add();// Adicionar novo amigo
                case "removeamigo":
                    return remover();// Remover amigo
            }
        }
        return false;
    }
